...
wandb.init(project="DHPC_morph_withgrad", entity="lucyzmf")  # , mode='disabled')
...
w_optimizer = create_w_optimizers(net, lr)
...
for epoch in range(epochs):
    ...
    for i, (image, label) in enumerate(train_loader):
        reset_w_grads(w_optimizer)
        net(image, inference_steps, istrain=True)

        errors_by_layer = compute_loss('weights', net, criterion)
        compute_gradient(net, errors_by_layer)

        w_update(w_optimizer)

        # TODO change total error computation
